...
<head>
	...
	<body>
		...
		//koneksi ke database
		$conn = new mysqli("localhost", "root", "", "psb2");
			...
			if ($cek->num_rows) {
				...
				if (strlen($password_baru) >= 5) {
					...
					if ($password_baru == $konfirmasi_password) {
						...
						$id_user 		= $_SESSION['nik']; //ini dari session saat login

						$update 		= $conn->query("UPDATE pengguna SET password='$password_baru' WHERE nik='$id_user'");
						if ($update) {
